...
def save_reduced_data(input_file,sp,unc,wave=None):
	...
	for i in np.arange(sp.shape[1]):
		...
		if i==0:
			...
			pyfits.writeto(os.path.splitext(input_file)[0]+'_reduced.fits', data,head)
		else:
			head['ORDER']=((i+1),'Order number')
	...
	pyfits.close(os.path.splitext(input_file)[0]+'_reduced.fits')#maybe
...
def Prepare(filepath):
	...
	for dirname, dirnames, filenames in os.walk('.'):
		...
		for filename in filenames:
			if '.fits' in filename:
				if 'drt' not in filename and '.old' not in filename and 'master' not in filename and 'reduced' not in filename:	
					...
					for j in keys:
						try:
							...
							if j in ['FIRMVSYS']:
								...
								try:
									temp=hdulist[0].header['DETECTOR']									
									#if temp in ['MITLL2','TEKTRONIX','MITLL2A','MITLL2a','EEV2','EEV2    ','MITLL3','TEKTRONIX_5']:
									hdulist[0].header[j]=(temp,'Duplicated from DETECTOR')
...
def reducebysubfolder(filepath):
	for f in os.listdir(filepath):
		...
		if os.path.isdir(child):
			...
			print'*** Reducing Stellar Exopsures ***'
			for j in range(len( filedata['REDUCEAS'] )):
